...
class PandaOp(IntEnum):
    ...
    CALL_ACC_SHORT  = 0x9a
    CALL_ACC        = 0x9b
    CALL_VIRT_SHORT = 0x9c
    CALL_VIRT       = 0x9d
    CALL_VIRT_RANGE = 0x9e
    CALL_VIRT_ACC_SHORT = 0x9f
    CALL_VIRT_ACC   = 0xa0
...
class IRInstruction:
    ...
    is_float: bool = False  # True for float/double operations
    cast_type: str = ""  # For cast operations: i2l, i2f, f2i, etc.
    # M3: Exception handling
    exception_type: str = ""  # Exception type for catch blocks
    try_start: int = 0  # Try block start offset
    try_end: int = 0    # Try block end offset
    catch_offset: int = 0  # Catch handler offset
...
class InstructionMapper:
    ...
    def _decode_dex(self, c: bytes, p: int, op: int) -> Tuple[Optional[IRInstruction], int]:
        ...
        # === FILLED NEW ARRAY ===
        elif op == DexOp.FILLED_NEW_ARRAY:
            # filled-new-array {vC, vD, vE, vF, vG}, type@BBBB
            b1 = c[p+1]  # arg count in high nibble, first reg in low nibble
            argc = (b1 >> 4) & 0xf
            first_reg = b1 & 0xf
            type_idx = struct.unpack_from("<H", c, p+2)[0]
            return IRInstruction(opcode="filled_new_array", dst=first_reg, imm=argc, field_ref=type_idx), 6
        elif op == DexOp.FILLED_NEW_ARRAY_RANGE:
            # filled-new-array/range {vCCCC .. vNNNN}, type@BBBB
            argc = c[p+1]  # number of elements
            type_idx = struct.unpack_from("<H", c, p+2)[0]
            first_reg = struct.unpack_from("<H", c, p+4)[0]  # first register in range
            return IRInstruction(opcode="filled_new_array_range", dst=first_reg, imm=argc, field_ref=type_idx), 6
        elif op == DexOp.FILL_ARRAY_DATA:
            # fill-array-data vAA, +BBBBBBBB
            reg = c[p+1]
            table_offset = struct.unpack_from("<i", c, p+2)[0]
            return IRInstruction(opcode="fill_array_data", src1=reg, imm=table_offset), 6
        ...
        # === SWITCH ===
        elif op == DexOp.PACKED_SWITCH:
            # packed-switch vAA, +BBBBBBBB
            reg = c[p+1]
            table_offset = struct.unpack_from("<i", c, p+2)[0]
            return IRInstruction(opcode="packed_switch", src1=reg, imm=table_offset), 6
        elif op == DexOp.SPARSE_SWITCH:
            # sparse-switch vAA, +BBBBBBBB
            reg = c[p+1]
            table_offset = struct.unpack_from("<i", c, p+2)[0]
            return IRInstruction(opcode="sparse_switch", src1=reg, imm=table_offset), 6
        ...
        # === ARRAY ACCESS ===
            ...
            # Check for wide variants (aget-wide=0x45)
            width = 64 if op == 0x45 else 32
            return IRInstruction(opcode="aget", dst=dst, src1=arr, src2=idx, width=width), 4
        elif 0x4b <= op <= 0x51:
            src = c[p+1]; arr = c[p+2]; idx = c[p+3]
            # Check for wide variants (aput-wide=0x4c)
            width = 64 if op == 0x4c else 32
            return IRInstruction(opcode="aput", src1=src, src2=arr, field_ref=idx, width=width), 4
        ...
        # === UNARY (neg/not/cast) ===
            ...
            b1 = c[p+1];
            # Map DEX conversion opcodes to specific cast types
            cast_types = {
                0x81: "i2l",   # int-to-long
                0x82: "i2f",   # int-to-float
                0x83: "i2d",   # int-to-double
                0x84: "l2i",   # long-to-int
                0x85: "l2f",   # long-to-float
                0x86: "l2d",   # long-to-double
                0x87: "f2i",   # float-to-int
                0x88: "f2l",   # float-to-long
                0x89: "f2d",   # float-to-double
                0x8a: "d2i",   # double-to-int
                0x8b: "d2l",   # double-to-long
                0x8c: "d2f",   # double-to-float
                0x8d: "i2b",   # int-to-byte
                0x8e: "i2c",   # int-to-char
                0x8f: "i2s",   # int-to-short
            }
            cast_type = cast_types.get(op, "cast")
            return IRInstruction(opcode="cast", dst=b1&0xf, src1=(b1>>4)&0xf, imm=op, cast_type=cast_type), 2

        # === ARITHMETIC 3-register (0x90-0xaf) ===
        elif 0x90 <= op <= 0xaf:
            dst = c[p+1]; s1 = c[p+2]; s2 = c[p+3]
            # Determine operation type and data type
            # DEX layout: 0x90-0x9a=int, 0x9b-0xa5=long, 0xa6-0xb0=float, 0xb1+ doesn't exist (0xaf is max)
            # Actually: 0x90-0x9f (16 slots, 11 used), 0xa0-0xaf (16 slots, 11 used)
            # Correct mapping per DEX spec:
            offset = op - 0x90
            if offset < 11:  # 0x90-0x9a: int
                op_idx = offset
                op_type = 0
            elif offset < 22:  # 0x9b-0xa5: long
                op_idx = offset - 11
                op_type = 1
            elif offset < 27:  # 0xa6-0xaa: float (only 5 ops: add, sub, mul, div, rem)
                op_idx = offset - 22
                op_type = 2
            else:  # 0xab-0xaf: double (only 5 ops: add, sub, mul, div, rem)
                op_idx = offset - 27
                op_type = 3

            names = ["add", "sub", "mul", "div", "rem", "and", "or", "xor", "shl", "shr", "ushr"]
            opcode_name = names[op_idx]

            # Determine width and float status
            width = 32
            is_float = False
            if op_type == 1:  # long
                width = 64
            elif op_type == 2:  # float
                is_float = True
                opcode_name = "f" + opcode_name
            elif op_type == 3:  # double
                width = 64
                is_float = True
                opcode_name = "d" + opcode_name

            return IRInstruction(opcode=opcode_name, dst=dst, src1=s1, src2=s2, width=width, is_float=is_float), 4

        # === ARITHMETIC 2addr (0xb0-0xcf) ===
        elif 0xb0 <= op <= 0xcf:
            b1 = c[p+1]; dst = b1&0xf; src = (b1>>4)&0xf
            base = op - 0xb0
            # DEX 2addr layout: 0xb0-0xba=int(11), 0xbb-0xc5=long(11), 0xc6-0xca=float(5), 0xcb-0xcf=double(5)
            if base < 11:  # int
                group = 0
                idx = base
                opcode_name = ["add","sub","mul","div","rem","and","or","xor","shl","shr","ushr"][idx]
                width = 32
                is_float = False
            elif base < 22:  # long
                group = 1
                ...
                width = 64
                is_float = False
            elif base < 27:  # float (0xc6-0xca)
                group = 2
                idx = base - 22
                opcode_name = "f" + ["add","sub","mul","div","rem"][idx]
                width = 32
                is_float = True
            else:  # double (0xcb-0xcf)
                group = 3
                idx = base - 27
                opcode_name = "d" + ["add","sub","mul","div","rem"][idx]
                width = 64
                is_float = True
            return IRInstruction(opcode=opcode_name, dst=dst, src1=dst, src2=src, width=width, is_float=is_float), 2
    ...
    def _emit_panda(self, ir: IRInstruction) -> bytes:
        ...
        if ir.opcode == "nop":
        ...
        elif ir.opcode in ("fadd", "fsub", "fmul", "fdiv", "frem"):
            # Float arithmetic: use 64-bit float ops
            out.append(PandaOp.LDA_64_v8); out.append(ir.src1 & 0xff)
            float_op_map = {
                "fadd": PandaOp.FADD2_64, "fsub": PandaOp.FSUB2_64,
                "fmul": PandaOp.FMUL2_64, "fdiv": PandaOp.FDIV2_64,
                "frem": PandaOp.FMOD2_64,
            }
            out.append(float_op_map[ir.opcode])
            out.append(ir.src2 & 0xff)
            out.append(PandaOp.STA_64_v8); out.append(ir.dst & 0xff)

        elif ir.opcode in ("dadd", "dsub", "dmul", "ddiv", "drem"):
            # Double arithmetic: same as float (64-bit)
            out.append(PandaOp.LDA_64_v8); out.append(ir.src1 & 0xff)
            float_op_map = {
                "dadd": PandaOp.FADD2_64, "dsub": PandaOp.FSUB2_64,
                "dmul": PandaOp.FMUL2_64, "ddiv": PandaOp.FDIV2_64,
                "drem": PandaOp.FMOD2_64,
            }
            out.append(float_op_map[ir.opcode])
            out.append(ir.src2 & 0xff)
            out.append(PandaOp.STA_64_v8); out.append(ir.dst & 0xff)
            ...
            # Load source value to accumulator
            if ir.cast_type in ("i2l", "i2f", "i2d", "f2l", "f2d", "d2l"):
                # Result is 64-bit
                out.append(PandaOp.LDA_v8); out.append(ir.src1 & 0xff)
                # For now, use mov as placeholder for actual conversion
                # In real implementation, would use proper conversion opcodes
                out.append(PandaOp.MOV_64_v16_v16)
                out.extend([ir.dst & 0xff, ir.src1 & 0xff])
            elif ir.cast_type in ("l2i", "f2i", "d2i", "d2f", "l2f"):
                # Result is 32-bit
                out.append(PandaOp.LDA_v8); out.append(ir.src1 & 0xff)
                out.append(PandaOp.MOV_v16_v16)
                out.extend([ir.dst & 0xff, ir.src1 & 0xff])
            else:
                # i2b, i2c, i2s - truncate to smaller type
                out.append(PandaOp.LDA_v8); out.append(ir.src1 & 0xff)
                # Use ANDI to mask (for byte: 0xFF, for char: 0xFFFF)
                if ir.cast_type == "i2b":
                    out.append(PandaOp.ANDI); out.append(0xff)
                elif ir.cast_type == "i2c":
                    out.append(PandaOp.ANDI); out.extend(struct.pack("<H", 0xffff))
                elif ir.cast_type == "i2s":
                    out.append(PandaOp.ANDI); out.extend(struct.pack("<H", 0xffff))
                out.append(PandaOp.STA_v8); out.append(ir.dst & 0xff)

        elif ir.opcode in ("packed_switch", "sparse_switch"):
            # Switch instructions - simplified to if-else chain
            # Load value to accumulator for comparison
            out.append(PandaOp.LDA_v8); out.append(ir.src1 & 0xff)
            # For now, emit a placeholder that will be patched later
            # In real implementation, would generate jump table or if-else chain
            out.append(PandaOp.JMP_imm32)
            out.extend(struct.pack("<i", ir.imm))  # Jump to switch table

        elif ir.opcode in ("filled_new_array", "filled_new_array_range"):
            # Array initialization - create array and fill elements
            # For now, simplified implementation
            out.append(PandaOp.NEWARR)
            out.append(((ir.dst & 0xf) << 4) | (ir.dst & 0xf))  # type and dst
            out.extend(struct.pack("<H", ir.field_ref & 0xffff))

        elif ir.opcode == "fill_array_data":
            # Fill array with static data
            out.append(PandaOp.LDA_v8); out.append(ir.src1 & 0xff)
            out.append(PandaOp.JMP_imm32)  # Jump to fill data
            out.extend(struct.pack("<i", ir.imm))
        ...
        elif ir.opcode == "catch":
            # M3: Exception catch block entry
            # Load exception type and prepare for handling
            out.append(PandaOp.MOVE_EXCEPTION)
            # Store to appropriate register based on catch block setup
            if ir.catch_offset > 0:
                out.append(PandaOp.JMP_imm32)
                out.extend(struct.pack("<i", ir.catch_offset))
    ...
    def map(self, dex_bytecode: bytes, tries: List[Tuple[int, int, str, int]] = None) -> Tuple[bytes, List[IRInstruction]]:
        """Full pipeline: DEX → IR → Panda ABC

        Args:
            dex_bytecode: DEX字节码
            tries: 可选的try-catch块列表 [(start_off, end_off, exception_type, handler_off)]
        """
        ir_list = self.dex_to_ir(dex_bytecode)

        # M3: Process try-catch blocks if provided
        if tries:
            ir_list = self._process_try_catches(ir_list, tries)

        abc = self.ir_to_abc(ir_list)
        return abc, ir_list

    def _process_try_catches(self, ir_list: List[IRInstruction],
                              tries: List[Tuple[int, int, str, int]]) -> List[IRInstruction]:
        """Process try-catch blocks and add exception handling IR"""
        result = []
        for ir in ir_list:
            result.append(ir)

        # Add catch block markers after the main IR
        for try_start, try_end, exc_type, catch_offset in tries:
            # Mark try block boundaries
            for ir in result:
                if ir.original_offset == try_start:
                    ir.try_start = try_start
                    ir.try_end = try_end

            # Add catch instruction
            catch_ir = IRInstruction(
                opcode="catch",
                exception_type=exc_type,
                catch_offset=catch_offset,
                original_offset=catch_offset
            )
            # Insert at appropriate position
            result.append(catch_ir)

        return sorted(result, key=lambda x: x.original_offset)
